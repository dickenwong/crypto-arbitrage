...
class Exchange {
  constructor(exchangeName) {
    ...
    this.availableCoins.forEach((coin) => {
      this.withdrawalFee[coin] = +this.exchange.fees.funding.withdraw[coin.toUpperCase()];
      ...
      assert(
        ...
        `${this.slugname} has no withdrawal for ${coin.toUpperCase()}.\n` +
        `${JSON.stringify(this.exchange.fees.funding.withdraw, null, 4)}`,
      );
    });
